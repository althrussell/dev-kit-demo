...
export function MaplibreMapView({
  ...
  useEffect(() => {
    ...
    const ensure = (fn: () => void) => {
      ...
      (map.getSource('assets') as GeoJSONSource | undefined)?.setData(assetsToFC(bundle.assets));
      (map.getSource('hazards') as GeoJSONSource | undefined)?.setData(hazardsToFC(bundle.hazards));
      (map.getSource('critical-customers') as GeoJSONSource | undefined)?.setData(
        criticalCustomersToFC(bundle.critical_customers),
      );
      (map.getSource('depots') as GeoJSONSource | undefined)?.setData(depotsToFC(bundle.depots));
      (map.getSource('mobile-gen') as GeoJSONSource | undefined)?.setData(
